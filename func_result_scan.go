...
func newScanDestByValues(sqlParamType map[reflect.Type]struct{}, columns []*sql.ColumnType, ret []reflect.Value) (destSlice []any, more bool, err error) {
	if len(ret) == 0 {
		return nil, more, fmt.Errorf("not scan dest")
	}
	...
	if len(ret) == 1 {
		...
		if _, ok := sqlParamType[t]; t.Kind() == reflect.Struct && !ok {
			...
			if t.Key().Kind() != reflect.String {
				return nil, more, fmt.Errorf("scan map key type not string")
			}
	...
	return destSlice, more, nil
}

func nextScan(ret []reflect.Value, scanRows []any) {
	if len(ret) == 1 {
		rt := ret[0].Type()
		var more bool
